...
vec3 getOffset(vec3 p) {
  ...
  vec3 temppos = rotation3dY(time * (0.8 + 1.4 * twist_scale) + length(pos.xz)) * p;
...
void main() {
  ...
  vec3 world_pos = rotation3dY(time * 0.2 * (0.1 + 0.5 * particle_size)) * pos;
  ...
  view_pos.xyz += position * size * (0.01 + 0.3 * particle_size);
